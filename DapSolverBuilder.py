...
JOINT_TRANSLATION = {"Rotation": "rev", "Linear Movement": "tran"}
...
class DapSolverBuilder:
    ...
    def __init__(self, obj):
        """ """
        self.obj = obj
        self.active_container = DapTools.getActiveContainer()
        self.doc_name = self.active_container.Document.Name
        ...
        self.list_of_force_ojects = DapTools.getListOfForceObjects()
        ...
        # NOTE: for calculations it does not matter what the origin of the projection plane is
        self.plane_origin = FreeCAD.Vector(
            0, 0, 0
        )  # NOTE assuming for now that plane moves through global origin

        # TODO run a number of validations
        # Validations to run:
    ...
    def processForces(self):
        ...
        for force_obj in self.list_of_force_ojects:
            ...
            if (
                force_obj.ForceTypes == "Rotational Spring"
                or force_obj.ForceTypes == "Rotational Spring Damper"
            ):
                if force_obj.Joint1 == "":
                    raise RuntimeError(
                        "Point 1 for " + str(force_obj.Label) + " was not defined"
                    )
            if (
                force_obj.ForceTypes == "Spring"
                or force_obj.ForceTypes == "Linear Spring Damper"
            ):
                if force_obj.Joint1 == "":
                    raise RuntimeError(
                        "Point 1 for " + str(force_obj.Label) + " was not defined"
                ...
                if force_obj.Joint2 == "":
                    raise RuntimeError(
                        "Point 2 for " + str(force_obj.Label) + " was not defined"
                    )
            force = {}
            if force_obj.ForceTypes == "Gravity":
                ...
                gravity_norm_rotated = (
                    self.global_rotation_matrix * gravity_norm_projected
                )
                force["type"] = "'weight'"
                force["gravity"] = gravity_mag
                force["wgt"] = (
                    "np.array([["
                    + str(gravity_norm_rotated.x)
                    + ", "
                    + str(gravity_norm_rotated.y)
                    + "]]).T"
                )
                # gravity_mag
            ...
            if (
                ...
                iIndex = self.addDapPointUsingJointCoordAndBodyLabel(
                    body1_index, body1, force_coord_1
                )
                if body1_index != 0:
                    self.obj.object_to_point[
                        str(force_obj.Label) + ":" + str(Joint1)
                    ] = (iIndex - 1)
                jIndex = self.addDapPointUsingJointCoordAndBodyLabel(
                    body2_index, body2, force_coord_2
                )
                if body2_index != 0:
                    self.obj.object_to_point[
                        str(force_obj.Label) + ":" + str(Joint2)
                    ] = (jIndex - 1)
    ...
    def processBodyInitialConditions(self):
        ...
        for i in range(len(self.body_objects)):
            ...
            body_objects = self.body_objects[i]
            ...
            self.body_init[body_label][
                "init_y"
            ] = body_objects.InitialVertical.getValueAs("m/s")
            self.body_init[body_label][
                "init_x"
            ] = body_objects.InitialHorizontal.getValueAs("m/s")
            self.body_init[body_label][
                "init_p"
            ] = body_objects.InitialAngular.getValueAs("rad/s")
        ...
        for i in range(len(self.joints)):
            ...
            if joint_type == "rev":
                if self.joints[i].Point1RelMov == "":
                    raise RuntimeError(
                        "Point for " + str(self.joints[i].Label) + " was not defined"
                    )
            if joint_type == "tran":
                if self.joints[i].Point1RelMov == "":
                    raise RuntimeError(
                        "Point 1 for " + str(self.joints[i].Label) + " was not defined"
                    )
                if self.joints[i].Point2RelMov == "":
                    raise RuntimeError(
                        "Point 2 for " + str(self.joints[i].Label) + " was not defined"
                    )
            ...
            if body1_index == 0 and body2_index == 0:
                raise RuntimeError(
                    "Both bodies attached to "
                    + str(self.joints[i].Label)
                    + " were defined as ground.\n"
            ...
            if joint_type == "rev":
                iIndex = self.addDapPointUsingJointCoordAndBodyLabel(
                    body1_index, body1, joint1_coord
                )
                self.obj.object_to_point[self.joints[i].Label] = iIndex - 1
                jIndex = self.addDapPointUsingJointCoordAndBodyLabel(
                    body2_index, body2, joint1_coord
                )
                self.addJoint(joint_type, iIndex, jIndex)
                ...
                jIndex = self.addDapPointUsingJointCoordAndBodyLabel(
                    body2_index, body2, joint2_coord
                )
                iUIndex, jUIndex = self.addUnitVectorBetweenTwoPoints(
                    self.joints[i].Label,
                    body1_index,
                    body2_index,
                    joint1_coord,
                ...
                if body1_index != 0:
                    self.obj.object_to_point[
                        str(self.joints[i].Label)
                        + ":"
                        + str(self.joints[i].Point1RelMov)
                    ] = (iIndex - 1)
                if body2_index != 0:
                    self.obj.object_to_point[
                        str(self.joints[i].Label)
                        + ":"
                        + str(self.joints[i].Point2RelMov)
                    ] = (jIndex - 1)
                self.addJoint(
                    joint_type, iIndex, jIndex, iUIndex=iUIndex, jUIndex=jUIndex
                )

    #  -------------------------------------------------------------------------
    def addUnitVectorBetweenTwoPoints(
        self,
        joint_label,
        body_index_1,
        body_index_2,
        ...
        if uVector.Length == 0:
            raise RuntimeError(
                'The two points defining the movement "'
                + str(joint_label)
            ...
            uvector_out["uLocal"] = (
                "np.array([[" + str(uVector.x) + ", " + str(uVector.y) + "]]).T"
            )
    ...
    #  -------------------------------------------------------------------------
    def addDapPointUsingJointCoordAndBodyLabel(
        self, body_index, body_label, point_coord
    ):
        """ """
        point = {}
    ...
    def extractDAPBodyIndex(self, body):
        ...
        if body == "Ground":
            ...
            if body_type == "Ground":
                ...
                body_index = (
                    self.moving_bodies.index(body) + 1
                )  # NOTE DAP.py is not 0 indexing
        return body_index
        # return
    ...
    def projectPointOntoPlane(self, point):
        ...
        projected_point = (
            point - (self.plane_norm * (point - self.plane_origin)) * self.plane_norm
        )
    ...
    def computeMomentOfInertia(self):
        ...
        FreeCAD.Console.PrintMessage(
            "List of bodies in J: " + str(self.list_of_bodies) + "\n"
        )
        for body_label in self.list_of_bodies:
            J_global_body = 0
            for shape_label in self.parts_of_bodies[body_label]:
                ...
                if self.parent_assembly_all[shape_label] != None:
                    shape_obj = DapTools.getAssemblyObjectByLabel(
                        self.doc, self.parent_assembly_all[shape_label], shape_label
                    )
                else:
                    shape_obj = self.doc.getObjectsByLabel(shape_label)[0]
                if shape_obj.Shape.ShapeType == "Compound":
                    if len(shape_obj.Shape.Solids) >= 1:
                        for i in range(len(shape_obj.Shape.Solids)):
                            J_global_body += self.computeShapeMomentOfInertia(
                                shape_obj.Shape.Solids[i], shape_label, body_label
                            )
                else:
                    J_global_body += self.computeShapeMomentOfInertia(
                        shape_obj.Shape, shape_label, body_label
                    )
    ...
    def computeShapeMomentOfInertia(self, shape_obj, shape_label, body_label):
        ...
        density = FreeCAD.Units.Quantity(
            self.material_dictionary[shape_label]["density"]
        ).getValueAs("kg/mm^3")
        ...
        if self.parent_assembly_all[shape_label] != None:
            parent_assembly_obj = self.doc.getObjectsByLabel(
                self.parent_assembly_all[shape_label]
            )[0]
            parent_assebly_placement_matrix = parent_assembly_obj.Placement.Matrix
            centre_of_gravity = (
                parent_assebly_placement_matrix * shape_obj.CenterOfMass * self.scale
            )
        else:
        ...
        for body_label in self.list_of_bodies:
            ...
            for shape_label in self.parts_of_bodies[body_label]:
                ...
                if self.parent_assembly_all[shape_label] != None:
                    ...
                    parent_assebly_placement_matrix = (
                        parent_assembly_obj.Placement.Matrix
                    )
                else:
                    shape_obj = self.doc.getObjectsByLabel(shape_label)[0]
                ...
                if shape_obj.Shape.ShapeType == "Compound":
                    if len(shape_obj.Shape.Solids) >= 1:
                        centre_of_gravity, volume = self.centerOfGravityOfCompound(
                            shape_obj
                        )
                else:
                ...
                if parent_assebly_placement_matrix != None:
                    centre_of_gravity = (
                        parent_assebly_placement_matrix * centre_of_gravity
                    )
                volume *= self.scale ** 3
            ...
            self.cog_of_body_projected[body_label] = self.projectPointOntoPlane(
                centre_of_gravity_global
            )
            self.total_mass_of_body[body_label] = total_mass
            self.cog_of_body_rotated[body_label] = (
                self.global_rotation_matrix * self.cog_of_body_projected[body_label]
            )
    ...
    def writeBodies(self):
        ...
        for i in range(len(self.moving_bodies)):
            ...
            fid.write(
                ...
                + str(i + 1)
                + ".m = "
                + str(self.total_mass_of_body[body_label])
                + "\n"
            )
            ...
            fid.write(
                ...
                + str(i + 1)
                + ".r = np.array([["
                + str(self.cog_of_body_rotated[body_label].x)
                + ", "
                + str(self.cog_of_body_rotated[body_label].y)
                + "]]).T\n"
            )
            ...
            fid.write(
                ...
                + ", "
                + str(self.body_init[body_label]["init_y"])
                + "]]).T\n"
            )
            fid.write(
                "B"
                + str(i + 1)
                + ".p_d = "
                + str(self.body_init[body_label]["init_p"])
                + "\n"
            )
            fid.write("\n")
    ...
    def solve(self):
        ...
        # dap_solver = os.path.join(cwd, "dap_solver", "DapTemp.py")
        ...
        DapTemp.folder = self.folder
        DapTemp.readInputFiles()
        DapTemp.initialize()
        DapTemp.t_initial = self.t_initial
        DapTemp.dt = self.reporting_time
        DapTemp.t_final = self.t_final
        FreeCAD.Console.PrintMessage("DAP solver started.\n")
        solution_success = DapTemp.solve()
        if solution_success:
            FreeCAD.Console.PrintMessage("Solver solved Successfully \n")
            DapTemp.writeOutputs()
            if DapTemp.write_success:
                FreeCAD.Console.PrintMessage(
                    "Results successfully loaded. Should now be able to animate and \
plot the generated results \n"
                )
                self.loadResults()
        else:
            FreeCAD.Console.PrintError(
                "There was an error solving the system. Proper error checking has not yet been\
included. \n"
            )
