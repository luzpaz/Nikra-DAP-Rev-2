...
class DapSolverBuilder():
    ...
    def __init__(self, obj):
        ...
        self.plane_origin = FreeCAD.Vector(0, 0, 0)  # NOTE assuming for now that plane moves through global origin
    ...
    def processBodyInitialConditions(self):
        ...
        for i in range(len(self.body_objects)):
            ...
            self.body_init[body_label]["init_y"] = body_objects.InitialVertical.getValueAs("m/s")
            self.body_init[body_label]["init_x"] = body_objects.InitialHorizontal.getValueAs("m/s")
            self.body_init[body_label]["init_p"] = body_objects.InitialAngular.getValueAs("rad/s")
    ...
    def computeShapeMomentOfInertia(self, shape_obj, shape_label, body_label):
        ...
        density = FreeCAD.Units.Quantity(self.material_dictionary[shape_label]["density"]).getValueAs("kg/mm^3")
    ...
    def computeCentreOfGravity(self):
        ...
        for body_label in self.list_of_bodies:
            ...
            for shape_label in self.parts_of_bodies[body_label]:
                ...
                density = FreeCAD.Units.Quantity(self.material_dictionary[shape_label]["density"]).getValueAs("kg/m^3")
                mass = density * volume
                total_mass += mass
